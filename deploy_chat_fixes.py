"""Deploy chat fixes:
1. telegram.py — timestamp now has Z suffix (UTC → local time display)
2. ChatCRM.tsx  — no auto-create clients, hide done chats, correct sort order
"""
...
print("\n--- TG bot status ---")
sx('curl -s http://127.0.0.1:8019/api/v1/telegram/status')
