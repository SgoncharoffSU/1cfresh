...
log('\n--- Frontend ---')
frontend_files = [
    'components/TelegramInboxPoller.tsx',
...
tg = sx('curl -s http://127.0.0.1:8018/api/v1/telegram/status')
log(f'TG status: {tg}')

msgs = sx('curl -s http://127.0.0.1:8018/api/v1/telegram/messages?limit=5')
log(f'TG messages: {msgs}')

client.close()
log('\nDone. http://159.194.225.55:3000/chats')
