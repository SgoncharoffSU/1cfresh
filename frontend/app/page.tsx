    ...
    <div className="relative w-full max-w-lg mx-auto">
      ...
      <div className="relative bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl overflow-hidden shadow-2xl">
        ...
        <div className="flex items-center gap-2 px-4 py-3 bg-white/10 border-b border-white/10">
          ...
          <span className="ml-3 text-white/50 text-xs font-mono">buhgsaas.ru/dashboard</span>
        </div>

        <div className="flex">
          {/* Sidebar */}
          <div className="w-14 bg-white/5 border-r border-white/10 py-4 flex flex-col items-center gap-3">
            {[FileText, Users, CalendarClock, BarChart3].map((Icon, i) => (
              <div key={i} className={`w-8 h-8 rounded-lg flex items-center justify-center ${i === 0 ? 'bg-blue-500' : 'bg-white/10'}`}>
          ...
          <div className="flex-1 p-4">
            <div className="text-white/70 text-[10px] font-medium mb-3 uppercase tracking-wider">Документы клиентов</div>
            ...
            {[
              ...
              <div key={i} className="flex items-center gap-3 py-2 border-b border-white/5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-white text-[11px] font-medium truncate">{row.name}</div>
                  <div className="text-white/40 text-[10px]">{row.doc}</div>
                </div>
                <span className="text-[9px] text-white/50 bg-white/10 px-2 py-0.5 rounded-full whitespace-nowrap">{row.status}</span>
              </div>
            ))}
...
function FeatureCard({
  ...
  return (
    <div className={`relative p-6 rounded-2xl border transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 ${accent ? 'bg-[#0f2444] border-[#1c3a5e] text-white' : 'bg-white border-slate-100 text-slate-900'}`}>
      <div className={`w-11 h-11 rounded-xl flex items-center justify-center mb-4 ${accent ? 'bg-blue-500/20' : 'bg-slate-100'}`}>
        <Icon size={20} className={accent ? 'text-blue-400' : 'text-[#1c3a5e]'} />
      </div>
      <h3 className={`text-base font-semibold mb-2 ${accent ? 'text-white' : 'text-slate-900'}`}>{title}</h3>
      <p className={`text-sm leading-relaxed ${accent ? 'text-white/60' : 'text-slate-500'}`}>{desc}</p>
    </div>
  );
}
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 antialiased">
      ...
      {/* ── Navbar ─────────────────────────────────────────────────────────── */}
      <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur border-b border-slate-200/60 shadow-sm' : 'bg-transparent'}`}>
        <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2.5">
            <LogoIcon className="h-8 w-auto" />
            <span className={`font-bold text-lg tracking-tight transition-colors ${scrolled ? 'text-[#1c3a5e]' : 'text-white'}`}>BuhgSaaS</span>
          </Link>

          {/* Desktop nav */}
          <nav className={`hidden md:flex items-center gap-6 text-sm font-medium transition-colors ${scrolled ? 'text-slate-600' : 'text-white/80'}`}>
          ...
          <div className="hidden md:flex items-center gap-3">
            {token ? (
              ...
              <>
                <Link href="/login" className={`text-sm font-medium px-4 py-2 rounded-lg transition-colors ${scrolled ? 'text-slate-700 hover:bg-slate-100' : 'text-white/90 hover:bg-white/10'}`}>
                  Войти
                </Link>
          ...
          {/* Mobile menu button */}
          <button onClick={() => setMenuOpen(!menuOpen)} className={`md:hidden p-2 rounded-lg ${scrolled ? 'text-slate-700' : 'text-white'}`}>
        ...
        {menuOpen && (
          <div className="md:hidden bg-white border-t border-slate-100 shadow-lg">
            <div className="max-w-6xl mx-auto px-5 py-4 flex flex-col gap-3">
              <a href="#features" onClick={() => setMenuOpen(false)} className="text-slate-600 text-sm py-2">Возможности</a>
              <a href="#how"      onClick={() => setMenuOpen(false)} className="text-slate-600 text-sm py-2">Как работает</a>
              <a href="#pricing"  onClick={() => setMenuOpen(false)} className="text-slate-600 text-sm py-2">Тарифы</a>
              ...
              {token ? (
                ...
                <>
                  <Link href="/login"    className="text-center text-sm font-medium text-slate-700 py-2 border border-slate-200 rounded-lg">Войти</Link>
      ...
      {/* ── Hero ───────────────────────────────────────────────────────────── */}
      <section className="relative min-h-screen flex items-center overflow-hidden bg-gradient-to-br from-[#060f1e] via-[#0f2444] to-[#1c3a5e]">
        {/* Grid bg */}
        ...
        <div className="relative max-w-6xl mx-auto px-5 pt-24 pb-20 w-full">
          <div className="grid lg:grid-cols-2 gap-14 items-center">
            ...
            <div>
              {/* Badge */}
              <div className="inline-flex items-center gap-2 bg-blue-500/15 border border-blue-400/30 rounded-full px-4 py-1.5 text-blue-300 text-xs font-medium mb-8">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <Link href="/login"
                  className="inline-flex items-center justify-center gap-2 bg-white/10 hover:bg-white/15 border border-white/20 text-white font-semibold px-7 py-3.5 rounded-xl text-base transition-all">
                  Войти в кабинет
                </Link>
              </div>
      ...
      <section className="bg-slate-50 border-y border-slate-100 py-8">
        <div className="max-w-6xl mx-auto px-5">
          <p className="text-center text-xs text-slate-400 uppercase tracking-widest mb-6">Создано для специалистов, работающих с</p>
          <div className="flex flex-wrap justify-center items-center gap-10">
            {[
              ...
              <div key={t} className="flex items-center gap-2 text-slate-400 text-sm font-medium">
            ...
            <div className="relative">
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                  <span className="text-sm font-semibold text-slate-700">Расписание — Июнь 2026</span>
                  <span className="text-xs text-slate-400">8 клиентов</span>
                </div>
                ...
                {[
                  ...
                  <div key={i} className={`flex items-center gap-4 px-6 py-3.5 border-b border-slate-50 ${row.done ? '' : 'opacity-50'}`}>
                    <div className="w-12 text-xs text-slate-400 font-mono flex-shrink-0">{row.date}</div>
                    <div className={`w-2 h-2 rounded-full flex-shrink-0 ${row.done ? 'bg-green-400' : 'bg-slate-300'}`} />
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-slate-800 truncate">{row.name}</div>
                      <div className="text-xs text-slate-400 truncate">{row.docs}</div>
                    </div>
                    {row.done && <CheckCircle2 size={14} className="text-green-500 flex-shrink-0" />}
                  </div>
                ))}

                <div className="px-6 py-4 bg-slate-50 flex items-center justify-between text-xs text-slate-400">
                  <span>Создано за месяц: 3 из 5</span>
                  <span className="text-green-600 font-medium">₽ 248 000</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="pricing" className="py-24 max-w-6xl mx-auto px-5">
        ...
        <div className="flex justify-center mb-10">
          <div className="inline-flex items-center gap-2 bg-green-50 border border-green-200 rounded-full px-5 py-2 text-green-700 text-sm font-medium">
        ...
        <div className="grid md:grid-cols-2 gap-6 max-w-3xl mx-auto">
          {[
            ...
            <div key={plan.name} className={`relative flex flex-col rounded-2xl border p-7 ${plan.accent ? 'bg-[#0f2444] border-[#1c3a5e] shadow-xl shadow-blue-900/20' : 'bg-white border-slate-100'}`}>
              {plan.accent && (
              ...
              <div className={`text-sm font-semibold mb-1 ${plan.accent ? 'text-blue-300' : 'text-slate-400'}`}>{plan.name}</div>
              <p className={`text-xs mb-4 ${plan.accent ? 'text-white/40' : 'text-slate-400'}`}>{plan.desc}</p>
              <div className={`mb-1 ${plan.accent ? 'text-white' : 'text-slate-900'}`}>
                <span className="text-3xl font-extrabold">{plan.price}</span>
                <span className={`text-sm ml-1 ${plan.accent ? 'text-white/40' : 'text-slate-400'}`}>{plan.sub}</span>
              </div>
              <div className={`text-xs mb-5 mt-1 ${plan.accent ? 'text-green-400' : 'text-green-600'}`}>
                15 дней бесплатно
              </div>
              <hr className={`mb-5 ${plan.accent ? 'border-white/10' : 'border-slate-100'}`} />
              <ul className="flex flex-col gap-3 flex-1">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-center gap-2.5 text-sm">
                    <CheckCircle2 size={14} className={plan.accent ? 'text-blue-400' : 'text-green-500'} />
                    <span className={plan.accent ? 'text-white/70' : 'text-slate-600'}>{f}</span>
                  </li>
                ))}
                {plan.overage && (
                  <li className="flex items-center gap-2.5 text-xs mt-1">
                    <ArrowRight size={12} className={plan.accent ? 'text-white/30' : 'text-slate-300'} />
                    <span className={plan.accent ? 'text-white/40' : 'text-slate-400'}>{plan.overage}</span>
                  </li>
                )}
              </ul>
              <Link href={plan.href}
                className={`mt-8 text-center text-sm font-semibold py-3 rounded-xl transition-all ${plan.accent ? 'bg-blue-500 hover:bg-blue-400 text-white shadow-lg' : 'bg-slate-900 hover:bg-slate-800 text-white'}`}>
                {plan.cta}
              </Link>
            </div>
          ))}
      ...
      {/* ── Final CTA ──────────────────────────────────────────────────────── */}
      <section className="py-24 bg-gradient-to-br from-[#060f1e] via-[#0f2444] to-[#1c3a5e] relative overflow-hidden">
        <div className="absolute inset-0 opacity-10"
        ...
        <div className="relative max-w-3xl mx-auto px-5 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Link href="/login"
              className="inline-flex items-center justify-center gap-2 border border-white/20 hover:bg-white/10 text-white font-semibold px-8 py-4 rounded-xl text-base transition-all">
              Уже есть аккаунт
            </Link>
          </div>
      ...
      {/* ── Footer ─────────────────────────────────────────────────────────── */}
      <footer className="bg-[#060f1e] border-t border-white/5 py-12">
        <div className="max-w-6xl mx-auto px-5">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
            ...
            <div className="lg:col-span-2">
              <div className="flex items-center gap-2.5 mb-4">
                <LogoIcon className="h-8 w-auto" />
                <span className="font-bold text-lg text-white">BuhgSaaS</span>
              </div>
            ...
            {/* Product */}
            <div>
              <h4 className="text-white/60 text-xs font-semibold uppercase tracking-wider mb-4">Продукт</h4>
              <ul className="flex flex-col gap-2.5">
                {['Возможности', 'Как работает', 'Тарифы', 'Безопасность'].map((t) => (
                  <li key={t}><a href="#" className="text-white/40 hover:text-white text-sm transition-colors">{t}</a></li>
                ))}
              </ul>
            </div>
            ...
            {/* Company */}
            <div>
              <h4 className="text-white/60 text-xs font-semibold uppercase tracking-wider mb-4">Компания</h4>
              <ul className="flex flex-col gap-2.5">
                {[
          ...
          <div className="border-t border-white/5 pt-8 flex flex-col sm:flex-row justify-between items-center gap-3">
            <p className="text-white/25 text-xs">© 2026 BuhgSaaS. Все права защищены.</p>
            <p className="text-white/25 text-xs">Создано для бухгалтеров России · 1С:Фреш OData</p>
          </div>
        </div>
      </footer>
    </div>
