...
export default function SuperAdminAuditPage() {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-5">
      <div>
        <h1 className="text-xl font-bold text-white">Журнал входов в аккаунты</h1>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center gap-3 text-slate-600 py-16">
          <ScrollText className="h-10 w-10 text-slate-800" />
            ...
            <div key={e.id} className="px-4 py-3 flex items-center gap-3 text-sm">
              <div className="flex-1 min-w-0">
                <p className="text-white">
                  ...
                  {' → '}
                  <span className="text-slate-300">{e.firm_name}</span>
                  <span className="text-slate-500"> (#{e.firm_id})</span>
                  {' как '}
                  <span className="text-slate-300">{e.target_user_name}</span>
                </p>
              </div>
