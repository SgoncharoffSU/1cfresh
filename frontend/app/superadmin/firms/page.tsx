  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white">Аккаунты бухгалтерий</h1>
          <p className="text-sm text-slate-500 mt-0.5">Всего: {firms.length}</p>
        </div>
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-3 py-2 rounded-lg border border-slate-700 bg-slate-900 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500 w-64"
          />
        </div>
      </div>
          ...
          {filtered.map((firm) => (
            <Link key={firm.id} href={`/superadmin/firms/${firm.id}`}
              ...
              <span className={cn(
                ...
                firm.subscription_status === 'active' ? 'bg-emerald-950 text-emerald-400'
                  : firm.subscription_status === 'trial' ? 'bg-blue-950 text-blue-400'
                  : 'bg-red-950 text-red-400',
              )}>
                {STATUS_LABEL[firm.subscription_status] ?? firm.subscription_status}
              </span>
              {!firm.is_active && (
                <span className="text-[10px] font-medium px-2 py-1 rounded-full bg-slate-800 text-slate-500 flex-shrink-0">
                  Отключена
                </span>
              )}
