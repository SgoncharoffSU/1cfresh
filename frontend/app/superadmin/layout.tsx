...
export default function SuperAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-[100dvh] flex-col overflow-hidden bg-slate-950 text-slate-100">
      <header className="flex-shrink-0 border-b border-slate-800 px-5 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <LogoIcon className="h-7 w-auto" />
          <div>
            <p className="text-sm font-bold leading-none">Суперадмин</p>
            <p className="text-[11px] text-slate-500 mt-0.5">Техподдержка glavinstrument</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-slate-400">{superAdmin?.name}</span>
          <button onClick={() => { logout(); router.push('/superadmin/login'); }}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs text-slate-400 hover:text-red-400 hover:bg-red-950/40 transition-colors">
